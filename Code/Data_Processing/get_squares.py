...
def crop_squares(imgs, flag=CLASSIC):
    ...
    for ind, img in enumerate(imgs):
        ...
        if np.abs(x0 - tlx) >= THRESHOLD_ROTATE or np.abs(x0 - blx) >= THRESHOLD_ROTATE:
            ...
            angle = np.arctan(slope)  # find the angle with the origin
            if angle > 0:
                angle = np.pi/2 - angle  # everything is inverted, due to (0, 0) being the top, left point, instead of the usual bottom left used in geometry
                rot_angle = 2*np.pi - angle  # rotation seems to be implicitly to the left
            else:
                angle = np.pi/2 + angle  # therefore, we find how much is left to
                rot_angle = angle
            ...
            img_rot = cv.warpAffine(src=img[y0:yn, x0:xn, :], M=rotation_mat,
                                    # dsize=((xn- x0), (yn - y0)))
                                    dsize=(int(np.max([tlx, trx, blx, brx]) - np.min([tlx, trx, blx, brx])),
            ...
            """
            For some reason, rotated images seem to have a weird deviation on their horizontal lines (after rotation),
            that's usually just small enough to disrupt my predictions, using the center patch idea. Warping the 
            perspective should hopefully solve this problem.
            """
        ...
        """
        old_perspective = np.array([[(tlx, tly), (trx, trry), (blx, bly), (brx, bry)]], np.float32)
        new_perspective = np.array([[(x0, y0), (xn - 1, y0), (x0, yn - 1), (xn - 1, yn - 1)]], np.float32)
        persp_transform = cv.getPerspectiveTransform(old_perspective, new_perspective)
